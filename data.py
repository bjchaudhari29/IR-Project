...
class QAData():
    ...
    def __init__(self):
        ...
        self.vocabulary = Vocabulary("./data/vocab.txt")
        self.dec_timesteps=300
        self.enc_timesteps=300
        self.answers = pickle.load(open("./data/answers_amazon.pkl",'rb'))
    ...
    def process_test_data(self, question, answers):
        ...
        for answer in answers:
            ...
            for word in answer.split(' '):
                handle = self.vocabulary.get(word)
                if handle:
                    new.append(word)
            answer_unpadded.append([self.vocabulary[word] for word in new])
        ...
        for word in question.split(' '):
            ...
            if handle:
                new.append(word)
        question = self.pad([[self.vocabulary[word] for word in new]] * len(answers), self.enc_timesteps)
